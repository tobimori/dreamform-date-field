...
if ($block->futureOnly()->toBool()) {
	$adpConfig['futureOnly'] = true;
}

if ($block->minDate()->isNotEmpty()) {
	$adpConfig['minDate'] = $block->minDate()->toString();
}

if ($block->maxDate()->isNotEmpty()) {
	$adpConfig['maxDate'] = $block->maxDate()->toString();
}

if ($block->defaultToToday()->toBool()) {
	$adpConfig['defaultToToday'] = true;
}

$adpConfig = A::merge($adpConfig, App::instance()->option('tobimori.dreamform-date-field.config') ?? []);
?>
...
<input <?= attr(A::merge($attr['input'], [
					...
					'readonly',
					'data-adp-init' => Json::encode($adpConfig)
				])) ?>>

<?php

if (App::instance()->option('tobimori.dreamform-date-field.script')) {
	snippet('dreamform/fields/date/script');
}
